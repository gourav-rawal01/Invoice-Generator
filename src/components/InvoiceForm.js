...
class InvoiceForm extends React.Component {
  ...
  handleAddEvent(evt) {
    ...
    this.setState(
      (prevState) => ({
        items: [...prevState.items, newItem], // Append new item to items array
      }),
      () => this.handleCalculateTotal() // Ensure total calculation happens after state update
    );
  }
  ...
  handleRowDel(item) {
    this.setState(
      (prevState) => ({
        items: prevState.items.filter((i) => i !== item), // Remove item from the array
      }),
      () => this.handleCalculateTotal() // Recalculate total after state update
    );
  }
  ...
  handleCalculateTotal() {
    ...
    let taxAmount = (subTotal * (taxRate / 100) || 0).toFixed(2);
    let discountAmount = (subTotal * (discountRate / 100) || 0).toFixed(2);
    let total = (subTotal - discountAmount + parseFloat(taxAmount)).toFixed(2);

    // Update calculated values in the state
    this.setState({
      subTotal,
